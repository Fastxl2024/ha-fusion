<script lang="ts">
	...
	// Vertaling van kleurenpalet
	const color_palette = $lang('color_palette');
	...
	// Check if the color palette entity exists
	$: colorPaletteEntityId = `select.${entity?.entity_id?.split('.')[1]}_${color_palette}`.toLowerCase();
	$: colorPaletteEntity = $states?.[colorPaletteEntityId];
	$: colorPaletteOptions = colorPaletteEntity?.attributes?.options as string[];
	
	function handleColorPaletteChange(event: CustomEvent<string>) {
		const selectedPalette = event.detail;
		callService($connection, 'select', 'select_option', {
			entity_id: colorPaletteEntityId,
			option: selectedPalette
		});
	}
	...
	{#if isOpen}
		<Modal>
			...
			{#if colorPaletteEntity}
				<h2>{$lang('color_palette')}</h2>
				<Select
					options={colorPaletteOptions.map(option => ({ id: option, label:option }))}
					value={colorPaletteEntity?.state}
					placeholder={$lang('select_color_palette')}
